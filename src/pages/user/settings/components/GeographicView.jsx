...
class GeographicView extends Component {
  componentDidMount = () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'userAndsettings/fetchProvince',
      });
    }
  };
  ...
  componentDidUpdate(props) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'userAndsettings/fetchCity',
...
export default connect(({ userAndsettings, loading }) => {
  const { province, city } = userAndsettings;
  return {
    province,
    city,
    loading: loading.models.userAndsettings,
  };
})(GeographicView);
